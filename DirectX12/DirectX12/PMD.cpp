...
void PMD::SetTexture(ID3D12Device* dev)
{
	for (UINT i = 0; i < material.size(); i++)
	{
		...
		{
			//読み込み
			result = BMP::GetInstance()->LoadBMP(i, FolderPath("サンプル/", material[i].textureFilePath), dev);
			if (FAILED(result))
			{
...
void PMD::SetTextureWIC(ID3D12Device * dev)
{
	for (UINT i = 0; i < material.size(); i++)
	{
		if (material[i].textureFilePath[0] != '\0')
		{
			result = BMP::GetInstance()->LoadTextureWIC(i, ChangeUnicode(FolderPath("サンプル/", material[i].textureFilePath).c_str()), dev);
			if (FAILED(result))
			{
				OutputDebugString(_T("テクスチャの読み込み：失敗\n"));
			}
